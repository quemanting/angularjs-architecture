...
var a, dp = {
	sh: {
		...
		ClipboardSwf: 'scripts/ZeroClipboard/ZeroClipboard.swf',
		Version: "1.5.1"
	}
};
...
a.GetMatches = function(b, c) {
	for (var d = null;
		(d = b.exec(this.code)) != null;) this.matches[this.matches.length] = new dp.sh.Match(d[0], d.index, c)
};
...
a.IsInside = function(b) {
	...
	for (var c = 0; c < this.matches.length; c++) {
		...
		if (d != null)
			if (b.index > d.index && b.index < d.index + d.length) return true
	}
	return false
};
...
dp.sh.Brushes.Xml.prototype.ProcessRegexList = function() {
	...
	for (b = new RegExp("([:\\w-.]+)\\s*=\\s*(\".*?\"|'.*?'|\\w+)*|(\\w+)", "gm");
		(a = b.exec(this.code)) != null;)
		if (a[1] != null) {
