...
(function() {
    ...
    var c = BMapLib.InfoBox = function(f, e, d) {
        ...
        this._opts.isClose = d.isClose ? true : false;
        this._opts.closeConfirm = d.closeConfirm?true:false;
        this._opts.btnToShow = d.btnToShow?true:false;
    }
    ;
    ...
    ;
    c.prototype.infohide = function() {
        if(!this._opts.closeConfirm){
            this.hide();
        }
        this._dispatchEvent(this, "infohide", {
            point: this._point
        })
    }
    ;
    c.prototype.saveBack = function() {
        this._dispatchEvent(this, "saveBack", {
            point: this._point
        })
    }
    ;
    c.prototype.deleteBack = function() {
        this._dispatchEvent(this, "deleteBack", {
            point: this._point
        })
    }
    ;
    ...
    a.object.extend(c.prototype, {
        _getCloseIcon: function() {
            var d = "<img src='" + this._opts.closeIconUrl + "' align='right' style='position:absolute;right:2px;cursor:pointer;margin:" + this._opts.closeIconMargin + "'/>";
            return d
        },
        _getBtnHtml: function(){
            var h = '<div class="infobox-btn-wrap"><button class="infobox-btn-item infobox-save" type="button">保存</button><button class="infobox-btn-item infobox-delete" type="button">删除</button></div>';
            return h;
        },
        _setContent: function(e) {
            ...
            if(this._opts.btnToShow){
                var c = this._getBtnHtml()
                this._div.innerHTML = this._div.innerHTML + c;
                this._addEventSave()
            }
            this._content = e;
            if(this._opts.isClose){
                this._addEventToClose()
            }else{
                this._addEventToHide()
            }
        },
        ...
        _addEventSave: function(){
            var ev = $(this._div).find('.infobox-save')[0],dv = $(this._div).find('.infobox-delete')[0];
            a.event.on(ev, "click", this._saveHandler());
            a.event.on(dv, "click", this._deleteHandler());
        },
        ...
        _addEventToHide: function(){
            var d = this;
            a.event.on(this._div.firstChild, "click", d._hideHandler());
            this._hasBindEventHide = true
        },
